...
from PyQt5.QtWidgets imoprt QApplication, QMainWindow
...
class Main(tk.Frame):
    def __init__(self, root, *args, **kwargs):
        ...
        self.root = root
        # Canvas Setup
        self.plotfig = Figure(figsize=(5, 4), dpi=100)

        self.canvas = FigureCanvasTkAgg(self.plotfig, master=root)
        ...
        toolbarFrame.grid(row=11, column=0, columnspan=9)
        self.toolbar = NavigationToolbar2Tk(self.canvas, toolbarFrame)
        self.toolbar.update()

        self.canvas.mpl_connect("key_press_event", self.on_key_press)

        title = tk.Label(self.root, text="Truss Calculator")
