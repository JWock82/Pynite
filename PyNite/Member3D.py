...
"""
# %%
from numpy import zeros, matrix, transpose, add, matmul
...
#%%
    ...
    def fer(self):
        ...
        for ptLoad in self.PtLoads:
            if ptLoad[0] == "Fx":
                ferVector = add(ferVector, PyNite.FixedEndReactions.FER_AxialPtLoad(ptLoad[1], ptLoad[2], self.L))
            elif ptLoad[0] == "Fy":
                ferVector = add(ferVector, PyNite.FixedEndReactions.FER_PtLoad(ptLoad[1], ptLoad[2], self.L, 1))
            elif ptLoad[0] == "Fz":
                ferVector = add(ferVector, PyNite.FixedEndReactions.FER_PtLoad(ptLoad[1], ptLoad[2], self.L, 2))
            elif ptLoad[0] == "Mx":
                ferVector = ferVector
            elif ptLoad[0] == "My":
                ferVector = add(ferVector, PyNite.FixedEndReactions.FER_Moment(ptLoad[1], ptLoad[2], self.L, 2))
            elif ptLoad[0] == "Mz":     
                ferVector = add(ferVector, PyNite.FixedEndReactions.FER_Moment(ptLoad[1], ptLoad[2], self.L, 1))
                
        # Sum the fixed end reactions for the distributed loads
        for distLoad in self.DistLoads:
            ferVector = add(ferVector, PyNite.FixedEndReactions.FER_LinLoad(distLoad.w1, distLoad.w2, distLoad.x1, distLoad.x2, distLoad.L, distLoad.Axis))
...
#%%
    ...
    def K(self):
        ...
        return transpose(self.T())*self.k()*self.T()
