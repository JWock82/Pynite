...
"""
# %%
from numpy import zeros, matrix, transpose
...
#%%
    ...
    def fer(self):
        ...
        for ptLoad in self.PtLoads:
            if ptLoad[0] == "Fx":
                ferVector = ferVector + PyNite.FixedEndReactions.FER_AxialPtLoad(ptLoad[1], ptLoad[2], self.L)
            elif ptLoad[0] == "Fy":
                ferVector = ferVector + PyNite.FixedEndReactions.FER_PtLoad(ptLoad[1], ptLoad[2], self.L, 1)
            elif ptLoad[0] == "Fz":
                ferVector = ferVector + PyNite.FixedEndReactions.FER_PtLoad(ptLoad[1], ptLoad[2], self.L, 2)
            elif ptLoad[0] == "Mx":
                ferVector = ferVector
            elif ptLoad[0] == "My":
                ferVector = ferVector + PyNite.FixedEndReactions.FER_Moment(ptLoad[1], ptLoad[2], self.L, 2)
            elif ptLoad[0] == "Mz":     
                ferVector = ferVector + PyNite.FixedEndReactions.FER_Moment(ptLoad[1], ptLoad[2], self.L, 1)
