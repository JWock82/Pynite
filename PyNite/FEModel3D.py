# %%
from numpy import matrix, zeros, empty, delete, insert, matmul, divide, add, subtract, nanmax, seterr, shape, array, vstack
...
#%%
    ...
    def AddPlate(self, Name, iNode, jNode, mNode, nNode, t, E, nu):
        ...
        # Create a new member
        newPlate = Plate3D(Name, self.GetNode(iNode), self.GetNode(jNode),
                           self.GetNode(mNode), self.GetNode(nNode), t, E, nu)
...
#%%
    ...
    def RemoveNode(self, Node):
        ...
        self.Members = [
            ...
            if member.iNode.Name != Node and member.jNode.Name != Node
        ]
...
#%%
    ...
    def DefineReleases(self,
        ...
        self.GetMember(Member).Releases = [
...
#%%
    ...
    def AddMemberDistLoad(self,
        ...
        self.GetMember(Member).DistLoads.append(
            (Direction, w1, w2, start, end, case))
...
#%%
    ...
    def __AuxList(self):
        ...
        D1_indices = [
        ]  # A list of the indices for the unknown nodal displacements
        D2_indices = [
        ]  # A list of the indices for the known nodal displacements
        D2 = [
        ]  # A list of the values of the known nodal displacements (D != None)
        ...
        for member in self.Members:
            ...
            for a in range(12):
                ...
                for b in range(12):
                    ...
                    # Now that 'm' and 'n' are known, place the term in the global stiffness matrix
                    Kg.itemset((m, n),
                               Kg.item((m, n)) + member_Kg.item((a, b)))
...
#%%
    ...
    def Analyze(self, check_statics=True):
        ...
        if self.LoadCombos == {}:
            ...
            self.LoadCombos['Combo 1'] = LoadCombo('Combo 1',
                                                   factors={'Case 1': 1.0})
        ...
        for combo in self.LoadCombos.values():
            ...
            FER1, FER2 = self.__Partition(self.FER(combo.name), D1_indices,
                                          D2_indices)

            # Get the partitioned global nodal force vector
            P1, P2 = self.__Partition(self.P(combo.name), D1_indices,
                                      D2_indices)

            # Check for global stability by determining if 'K11' is singular
            print('...Checking global stability for load combination',
                  combo.name)
            if K11.shape == (0, 0):
                ...
                print(
                    '...Calculating global displacement vector for load combination',
                    combo.name)
            ...
            for node in self.Nodes:
                ...
                if D2_indices.count(node.ID * 6 + 0) == 1:
                    D.itemset((node.ID * 6 + 0, 0),
                              D2[D2_indices.index(node.ID * 6 + 0), 0])
                else:
                    D.itemset((node.ID * 6 + 0, 0),
                              D1[D1_indices.index(node.ID * 6 + 0), 0])

                if D2_indices.count(node.ID * 6 + 1) == 1:
                    D.itemset((node.ID * 6 + 1, 0),
                              D2[D2_indices.index(node.ID * 6 + 1), 0])
                else:
                    D.itemset((node.ID * 6 + 1, 0),
                              D1[D1_indices.index(node.ID * 6 + 1), 0])

                if D2_indices.count(node.ID * 6 + 2) == 1:
                    D.itemset((node.ID * 6 + 2, 0),
                              D2[D2_indices.index(node.ID * 6 + 2), 0])
                else:
                    D.itemset((node.ID * 6 + 2, 0),
                              D1[D1_indices.index(node.ID * 6 + 2), 0])

                if D2_indices.count(node.ID * 6 + 3) == 1:
                    D.itemset((node.ID * 6 + 3, 0),
                              D2[D2_indices.index(node.ID * 6 + 3), 0])
                else:
                    D.itemset((node.ID * 6 + 3, 0),
                              D1[D1_indices.index(node.ID * 6 + 3), 0])

                if D2_indices.count(node.ID * 6 + 4) == 1:
                    D.itemset((node.ID * 6 + 4, 0),
                              D2[D2_indices.index(node.ID * 6 + 4), 0])
                else:
                    D.itemset((node.ID * 6 + 4, 0),
                              D1[D1_indices.index(node.ID * 6 + 4), 0])

                if D2_indices.count(node.ID * 6 + 5) == 1:
                    D.itemset((node.ID * 6 + 5, 0),
                              D2[D2_indices.index(node.ID * 6 + 5), 0])
                else:
                    D.itemset((node.ID * 6 + 5, 0),
                              D1[D1_indices.index(node.ID * 6 + 5), 0])
        ...
        for combo in self.LoadCombos.values():
            ...
            while convergence == False and divergence == False:
                ...
                if iter_count == 1:
                    ...
                    K11, K12, K21, K22 = self.__Partition(
                        self.K(), D1_indices,
                        D2_indices)  # Initial stiffness matrix
                    FER1, FER2 = self.__Partition(
                        self.FER(combo.name), D1_indices,
                        D2_indices)  # Fixed end reactions
                    P1, P2 = self.__Partition(self.P(combo.name), D1_indices,
                                              D2_indices)  # Nodal forces
                    ...
                    K11, K12, K21, K22 = self.__Partition(
                        self.K(), D1_indices,
                        D2_indices)  # Initial stiffness matrix
                    Kg11, Kg12, Kg21, Kg22 = self.__Partition(
                        self.Kg(combo.name), D1_indices,
                ...
                if K11.shape == (0, 0):
                    ...
                    D1 = solve(K11,
                               subtract(subtract(P1, FER1), matmul(K12, D2)))
                ...
                if iter_count != 1:
                    ...
                    if abs(1 - nanmax(divide(prev_results, D1))) <= tol:
                        convergence = True
                        print('...P-Delta analysis converged after ' +
                              str(iter_count) + ' iterations.')
                    # Check for divergence
                    elif iter_count > max_iter:
                        divergence = True
                        print(
                            '...P-Delta analysis failed to converge after 30 iterations.'
                        )
...
#%%
    ...
    def __CheckStatics(self):
        ...
        statics_table = []
        table_header = [
            'Load Combination', 'Sum FX', 'Sum RX', 'Sum FY', 'Sum RY',
            'Sum FZ', 'Sum RZ', 'Sum MX', 'Sum RMX', 'Sum MY', 'Sum RMY',
            'Sum MZ', 'Sum RMZ'
        ]
