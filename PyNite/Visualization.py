...
class VisDeformedMember():
  def __init__(self, member, nodes, scalefactor, textHeight):
    ...
    for node in nodes:
      ...
      if node.Name == member.iNode.Name:
       ...
       # Check to see if the current node is the j-node
          ...
          newXj = (node.X) + (node.DX)
          newYj = (node.Y) + (node.DY)
          newZj = (node.Z) + (node.DZ)

       
    dy_plot=empty((0,3))
    for i in range(20):
      
      #Relative displacements in y direction (local reference system)
    ...
    for i in range(20):
      ...
      #Relative displacements in z direction (local reference system)
    ...
    x_plot=empty((0,3)) 
    for i in range(20):
      x=[[newXi,newYi,newZi]] + (L/19 *i)*cos_x
      x_plot=append(x_plot,x,axis=0)
        
    #This is the vector to plot        
