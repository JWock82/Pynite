...
class MITC4:
    ...
    def B_kappa(self, r, s):
        ...
        dH = matmul(
            ...
            1
            / 4
            * array([[1 + s, -1 - s, -1 + s, 1 - s], [1 + r, 1 - r, -1 + r, -1 - r]]),
        )
    ...
    def B_gamma_MITC4(self, r, s):
        ...
        # the local x-axis.
        r_axis = array(
            [(x1 + x4) / 2 - (x2 + x3) / 2, (y1 + y4) / 2 - (y2 + y3) / 2, 0]
        ).T
        s_axis = array(
            [(x1 + x2) / 2 - (x3 + x4) / 2, (y1 + y2) / 2 - (y3 + y4) / 2, 0]
        ).T
    ...
    def Cb(self):
        ...
        Cb = (
            ...
            * h**3
            / (12 * (1 - nu**2))
            * array([[1, nu, 0], [nu, 1, 0], [0, 0, (1 - nu) / 2]])
        )
    ...
    def k_m(self):
        ...
        k = t * (
            matmul(B1.T, matmul(Cm, B1)) * det(self.J(gp, gp))
