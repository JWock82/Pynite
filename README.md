...
* Members are now stored as a dictionary for faster computing and easier user access. Instead of using the `FEModel3D.GetMember('my_member_name')` method you can now alternatively access a member from the dictionary using the syntax `FEModel3D.Members['my_member_name']`.
