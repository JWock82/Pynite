...
* VTK: used for visualization - Note that VTK is a little picky about which version of Python you are running. You must run a 64 bit installation of Python, rather than a 32 bit version. Kitware is usually pretty quick, but sometimes they lag a little getting their VTK Python builds out, so VTK sometimes doesn't support the latest version of Python. If you're having trouble installing VTK, you can see which versions of Python are supported by visiting VTK's PyPI page and clicking the "Download files" button. VTK does not need to be installed if you don't plan to use the visualization tools built into PyNite.
