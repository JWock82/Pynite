...
def test_shear_walls():
    ...
    for wall_name in ["W1", "W2"]:
        ...
        model.shear_walls[wall_name].add_opening(
            "Door 1", x_start=2, y_start=0, width=4, height=12, tie=None
        )
        model.shear_walls[wall_name].add_opening(
            "Window 1", x_start=8, y_start=8, width=4, height=4, tie=None
        )
        model.shear_walls[wall_name].add_opening(
            "Window 2", x_start=14, y_start=8, width=4, height=4, tie=None
        )
        model.shear_walls[wall_name].add_opening(
            "Door 2", x_start=20, y_start=0, width=4, height=12, tie=None
        )
...
def test_piers_and_coupling_beams():
    ...
    model.shear_walls["Wall1"].add_opening(
        "Door", x_start=5, y_start=0, width=10, height=10, tie=None
    )
    ...
    assert round(V1, 3) == -50 and round(V3, 3) == -50, (
        "Failed shear wall pier statics check (sum of shears != 0)."
    )
    assert round(P1, 3) == -round(P3, 3), (
        "Failed shear wall pier statics check (sum of axial forces != 0.)"
    )
    assert round(M1, 3) == round(M3, 3), (
        "Failed shear wall pier statics check (sum of moments != 0)."
    )
...
def test_cracked_rect_shear_wall():
    ...
    mesh_size = 1
    sw.add_rectangle_mesh(
        "MSH1", mesh_size, L, H, t, "Concrete", ky_mod=0.35, element_type="Rect"
    )
    sw.meshes["MSH1"].generate()
...
def test_shear_wall_openings():
    ...
    model.meshes["MSH1"].add_rect_opening(
        name="Door 1", x_left=2 * 12, y_bott=0 * 12, width=4 * 12, height=12 * 12
    )

    # Add a 4' wide x 4' tall window opening to the mesh
    model.meshes["MSH1"].add_rect_opening(
        name="Window 1", x_left=8 * 12, y_bott=8 * 12, width=4 * 12, height=4 * 12
    )

    # Add another 4' wide x 4' tall window opening to the mesh
    model.meshes["MSH1"].add_rect_opening(
        name="Window 2", x_left=14 * 12, y_bott=8 * 12, width=4 * 12, height=4 * 12
    )

    # Add another 4' wide x 12' tall door opening to the mesh
    model.meshes["MSH1"].add_rect_opening(
        name="Door 2", x_left=20 * 12, y_bott=0 * 12, width=4 * 12, height=12 * 12
    )
