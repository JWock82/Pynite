...
class TestLoads(unittest.TestCase):
    ...
    def test_member_self_weight(self):
        ...
        cont_beam.add_section(
            "Section", 10 / 12**2, 23.3 / 12**4, 340 / 12**4, 0.569 / 12**4
        )
    ...
    def test_member_self_weight_local_direction(self):
        ...
        for direction in local_directions:
            with self.assertRaises(
                ValueError, msg=f"Local direction '{direction}' should raise ValueError"
            ):
                model.add_member_self_weight(
                    global_direction=direction, factor=1, case="D"
                )
        ...
        for direction in global_directions:
            try:
                ...
                self.fail(
                    f"add_member_self_weight raised ValueError unexpectedly with global direction {direction}."
                )
