...
def create_3_support_model():
    ...
    continuous_beam.def_support(
        "N1", True, True, True, True, True, False
    )  # Constrained for torsion at 'N1'
    continuous_beam.def_support(
        "N2", False, True, False, False, False, False
    )  # Not constrained for torsion at 'N2'
    continuous_beam.def_support(
        "N3", False, True, False, False, False, False
    )  # Same for 'N3'
...
def test_3_support_beam_moments():
    ...
    assert math.isclose(max(member.moment_array("Mz", 11, "1.0D")[1]), 18000.00), (
