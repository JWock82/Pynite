...
def test_PCA_7_quad():
    ...
    tank_model.add_cylinder_mesh(
        "MSH1", mesh_size, R, H, t, "Concrete", 1, 1, center, axis, element_type="Quad"
    )
    ...
    Sx = (
        max([element.membrane(0, 0)[0, 0] for element in tank_model.quads.values()]) * t
    )
...
def test_PCA_7_rect():
    ...
    tank_model.add_cylinder_mesh(
        "MSH1", mesh_size, R, H, t, "Concrete", 1, 1, center, axis, element_type="Rect"
    )
    ...
    Sx = (
        max(
            [
                element.membrane(element.width() / 2, element.height() / 2)[0, 0]
                for element in tank_model.plates.values()
            ]
        )
        * t
