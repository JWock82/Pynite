...
def test_hydrostatic_plate():
    ...
    plate_model.add_rectangle_mesh(
        "MSH1", mesh_size, a, b, t, "Concrete", kx_mod=1, ky_mod=1, element_type="Rect"
    )
    ...
    for element in plate_model.plates.values():
        Yavg = (
            element.i_node.Y + element.j_node.Y + element.m_node.Y + element.n_node.Y
        ) / 4
        p = 62.4 * (b - Yavg)
    ...
    # values.
    assert abs(DZ_calcd / DZ_expected - 1) < 0.15, (
...
def test_hydrostatic_quad():
    ...
    quad_model.add_rectangle_mesh(
        "MSH1", mesh_size, a, b, t, "Concrete", kx_mod=1, ky_mod=1, element_type="Quad"
    )
...
def test_circular_hopper():
    ...
    rndr.render_model(
        interact=False
    )  # interact=False prevents the test from stalling in a headless environment
