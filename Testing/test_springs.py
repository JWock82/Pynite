...
def test_nodal_springs():
    ...
    assert model.nodes["N1"].DX["Combo 1"] == -1, (
        "Faied nodal spring displacement test in X direction."
    )
    assert model.nodes["N1"].DY["Combo 1"] == -1, (
        "Faied nodal spring displacement test in Y direction."
    )
    assert model.nodes["N1"].DZ["Combo 1"] == -1, (
        "Faied nodal spring displacement test in Z direction."
    )
    assert model.nodes["N1"].RX["Combo 1"] == -1, (
        "Faied nodal spring displacement test in RX direction."
    )
    assert model.nodes["N1"].RY["Combo 1"] == -1, (
        "Faied nodal spring displacement test in RY direction."
    )
    assert model.nodes["N1"].RZ["Combo 1"] == -1, (
        "Faied nodal spring displacement test in RZ direction."
    )
