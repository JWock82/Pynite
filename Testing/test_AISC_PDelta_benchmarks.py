...
"""
AISC's benchmark problems used to determine if a second order analysis procedure is rigorous enough.
"""

def test_AISC_benchmark_old():
    ...
    assert math.isclose(calculated_moment/Mmax, 1.0, rel_tol=0.01), f"Calculated moment {calculated_moment} is not within 1% of expected moment {Mmax}"
    assert math.isclose(calculated_displacement/(ymax*12), 1.0, rel_tol=0.01), f"Calculated displacement {calculated_displacement} is not within 1% of expected displacement {ymax*12}"

def test_AISC_benchmark_case1():
    ...
    for i, val in enumerate(Mmid_calculated):
        assert math.isclose(Mmid_calculated[i]/Mmid_expected[i], 1.0, rel_tol=0.03), f"Calculated moment {Mmid_calculated[i]} is not within 3% of expected moment {Mmid_expected[i]}"
        assert math.isclose(dmid_calculated[i]/dmid_expected[i], 1.0, rel_tol=0.05), f"Calculated deflection {dmid_calculated[i]} is not within 5% of expected deflection {dmid_expected[i]}"

def test_AISC_benchmark_case2():
