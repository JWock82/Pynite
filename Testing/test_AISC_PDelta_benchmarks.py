...
class Test_AISC_Benchmark(unittest.TestCase):
    ...
    def test_AISC_benchmark_case1(self):
        ...
        for combo in column.load_combos.values():
            Mmid_calculated.append(
                -column.members["M1"].moment("Mz", 14, combo.name) * 12
            )
            dmid_calculated.append(
                -column.members["M1"].deflection("dy", 14, combo.name) * 12
            )
    ...
    def test_AISC_benchmark_case2(self):
        ...
        dtip_calculated = []
        for combo in column.load_combos.values():
            Mbase_calculated.append(
                -column.members["M1"].moment("Mz", 0, combo.name) * 12
            )
            dtip_calculated.append(
                -column.members["M1"].deflection("dy", 28, combo.name) * 12
            )
