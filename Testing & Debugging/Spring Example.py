...
BracedFrame.AddSpring('Spring1','N1', 'N1s', 10000, tension_only=True, comp_only=False)
BracedFrame.AddSpring('Spring2','N4', 'N4s', 10000, tension_only=False, comp_only=True) # The structure would be unstable if this was tension only

# Release the brace ends to form an axial member
BracedFrame.DefineReleases('Brace1', Ryi=True, Rzi=True, Ryj=True, Rzj=True)

# Springs only carry axial loads, nothing else, so we'll need to stabilize them in the
# other directions.
# The bases of the columns will be supported by the springs vertically. For the other
# directions (horizontally and about the Y-axis) we'll need to provide supports.
...
# Fix the nodes supporting the bottoms of the springs. Note that even though we're fixing
# these nodes, the only reactions these supports will carry will be in the Y-direction, due
# to the fact that the spring only has stiffness in that direction. We fix the node so that
# it's not free to spin or translate in the other directions however. If we didn't the node
# would be unstable and the model would crash. This is a concept a lot of structural engineers
# struggle to grasp when modeling with finite elements, and PyNite is unforgiving in this regard.
# Every degree of freedom (3 translations and 3 rotations) at every node must be stabilized by
# one or more members, or a support. The spring can only stabilize the nodes in the vertical
# direction, so we're adding the other supports in.
...
#***********************************************************************************
# Note that spring's do not show up in the rendering engine yet, but they are there.
#***********************************************************************************
