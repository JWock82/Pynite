...
class VTKWriter:
    ...
    def write_to_vtk(self, path: str):
        ...
        if len(self.model.plates) > 0:
            print(
                "[WARNING] Plate export is not supported at this time. If needed, you can substitude them by Quad3D Elements."
            )
    ...
    def _write_node_data(self, path: str):
        ...
        for combo in self.model.load_combos.keys():
            ...
            for node_name, node_id in node_ids.items():
                ...
                reaction_constraints.InsertTuple6(
                    node_id,
                    int(node.support_DX),
                    int(node.support_DY),
                    int(node.support_DZ),
                ...
                displacements.InsertTuple3(
                    node_id, node.DX[combo], node.DY[combo], node.DZ[combo]
                )  # type: ignore
                forces.InsertTuple3(
                    node_id, node.RxnFX[combo], node.RxnFY[combo], node.RxnFZ[combo]
                )
                moments.InsertTuple3(
                    node_id, node.RxnMX[combo], node.RxnMY[combo], node.RxnMZ[combo]
                )
            ...
            if len(member.sub_members) == 0:
                ...
                line.GetPointIds().SetId(
                    ...
                    points.InsertNextPoint(
                        member.i_node.X, member.i_node.Y, member.i_node.Z
                    ),
                )
                line.GetPointIds().SetId(
                    1,
                    points.InsertNextPoint(
                        member.j_node.X, member.j_node.Y, member.j_node.Z
                    ),
                )
        ...
        for combo in self.model.load_combos.keys():
            ...
            for line_range, subm, line in submembers:
                for i, x in enumerate(line_range):
                    ...
                    deflection = np.array(
                        [
                            float(subm.deflection(direction, xl, combo))
                    ...
                    m = np.array(
                        [
                            subm.torque(xl, combo),
                            subm.moment("My", xl, combo),
                            subm.moment("Mz", xl, combo),
                        ]
                    )
                    ...
                    s = np.array(
                        [
                            subm.axial(xl, combo),
                            subm.shear("Fy", xl, combo),
                            subm.shear("Fz", xl, combo),
                        ]
                    )
                    ...
                    sig_b = np.array(
                        [
                            0,
                            subm.moment("My", xl, combo) / sec.Iy,
                            subm.moment("Mz", xl, combo) / sec.Iz,
                        ]
                    )  # type: ignore
    ...
    def _write_quad_data(self, path: str):
        ...
        )  # this holds all data and gets written to .vtk at the end
        points = vtk.vtkPoints()  # this holds all Point data
        quads = (
            vtk.vtkCellArray()
        )  # this holds all cell data. A cell is in this case the vtkBiQuadraticQuads
        ...
        for quad in self.model.quads.values():
            ...
            for i, subquad in enumerate(quad_references[quad.name]):
                for vert_id in range(9):
                    ...
                    xi = (
                        coordinates[vert_id][0] * 0.5 + (i % 2) - 0.5
                    )  # Scale and shift xi
                    eta = (
                        coordinates[vert_id][1] * 0.5 + (i // 2) - 0.5
                    )  # Scale and shift eta
                    point_coords = self._interpolate_quad_corner_data(
                        i_coords, j_coords, m_coords, n_coords, xi, eta
                    )

                    # set the Vertex ID of the subquad to the global ID of the newly created point at the calculated coordinates
                    subquad.GetPointIds().SetId(
                        vert_id, points.InsertNextPoint(point_coords)
                    )  # type: ignore
            ...
            for quad_name, subquads in quad_references.items():
                ...
                for i, subquad in enumerate(subquads):
                    ...
                    for vert_id in range(9):
                        ...
                        di = np.array(
                            [
                                quad.i_node.DX[combo],
                                quad.i_node.DY[combo],
                                quad.i_node.DZ[combo],
                            ]
                        )
                        dj = np.array(
                            [
                                quad.j_node.DX[combo],
                                quad.j_node.DY[combo],
                                quad.j_node.DZ[combo],
                            ]
                        )
                        dm = np.array(
                            [
                                quad.m_node.DX[combo],
                                quad.m_node.DY[combo],
                                quad.m_node.DZ[combo],
                            ]
                        )
                        dn = np.array(
                            [
                                quad.n_node.DX[combo],
                                quad.n_node.DY[combo],
                                quad.n_node.DZ[combo],
                            ]
                        )

                        d = self._interpolate_quad_corner_data(
                            di, dj, dm, dn, xi[vert_id], eta[vert_id]
                        )
                        ...
                        membrane_loc.InsertTuple3(
                            subquad.GetPointId(vert_id), *p_membrane
                        )  # type: ignore
                        ...
                        membrane_glob.InsertTuple3(
                            subquad.GetPointId(vert_id), *p_membrane_glob
                        )  # type: ignore
                        moments_glob.InsertTuple3(
                            subquad.GetPointId(vert_id), *p_moment_glob
                        )  # type: ignore
                        shear_glob.InsertTuple3(
                            subquad.GetPointId(vert_id), *p_shear_glob
                        )  # type: ignore
    ...
    @staticmethod
    def _interpolate_member_data(
        p1: np.ndarray, p2: np.ndarray, x: float
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def _interpolate_quad_corner_data(
        i: float | np.ndarray,
        j: float | np.ndarray,
        m: float | np.ndarray,
        n: float | np.ndarray,
