...
class ShearWall:
    ...
    def add_support(
        ...
        if (
            ...
            raise Exception(
                f"Support for shear wall `{self.name}` falls outside the extents of the wall and is invalid."
            )
    ...
    def add_story(
        ...
        self.model.add_load_combo(
            "Stiffness: " + story_name, {story_name: 1.0}, "stiffness"
        )
    ...
    def generate(self) -> None:
        ...
        for opng in self._openings:
            ...
            self.model.meshes[self.name].add_rect_opening(
                name, x_start, y_start, width, height
            )
            ...
            if AE is not None:
                ...
                self.model.add_node(
                    i_node_name,
                    *self._local2global(x_start, y_start + height, 0, self.plane),
                )
                ...
                self.model.add_node(
                    j_node_name,
                    *self._local2global(
                        x_start + width, y_start + height, 0, self.plane
                    ),
                )

                tie_name = self.model.unique_name(self.model.Members, "Tie ")
                self.model.add_member(
                    tie_name, i_node_name, j_node_name, "Tie", 1, 1, 1, AE
                )
        ...
        for i, flg in enumerate(self._flanges):
            ...
            if side == "+z":
                ...
                flg_x_control = [
                    val
                    for val in z_control
                    if round(val, 10) >= 0 and round(val, 10) <= b
                ]
            else:
                z = -b
                flg_x_control = [
            ...
            flg_y_control = [
                y - y_start
                for y in y_control
                if round(y, 10) >= round(y_start, 10)
                and round(y, 10) <= round(y_end, 10)
            ]
        ...
        for support in self._supports:
            ...
            for node in self.model.nodes.values():
                ...
                if (
                    isclose(y, elevation)
                    and round(x, 10) >= round(x_start, 10)
                    and round(x, 10) <= round(x_end, 10)
                ):
                    self.model.def_support(
                        node.name, True, True, True, True, True, True
                    )
        ...
        for story in self._stories:
            ...
            for node in node_list:
                ...
                for shear in self._shears:
                    ...
                    if story == story_name:
                        self.model.add_node_load(
                            node.name, direction, force / num_nodes, case
                        )
                ...
                for axial in self._axials:
                    ...
                    if story == story_name:
                        self.model.add_node_load(
                            node.name, "FY", -force / num_nodes, case
                        )
    ...
    def _identify_piers(self) -> None:
        ...
        for i in range(len(x_vals) - 1):
            ...
            x = x_vals[i]
            y = 0
            self.piers["P" + str(i + 1)] = Pier(
                "P" + str(i + 1), x, y, width, height, self
            )
        ...
        for pier in self.piers.values():
            for i in range(len(y_vals) - 1):
                ...
                x = pier.x
                y = y_vals[i]
                new_piers["P" + str(pier_count)] = Pier(
                    "P" + str(pier_count), x, y, width, height, self
                )
                pier_count += 1
        ...
        for pier in self.piers.values():
            ...
            for opng in self._openings:
                if (
                    round(pier.x, 10) >= round(opng[1], 10)
        ...
        for plate in self.model.quads.values():
            xi, yi, zi = _global2local(
                plate.i_node.X, plate.i_node.Y, plate.i_node.Z, self.origin, self.plane
            )
            xj, yj, zj = _global2local(
                plate.j_node.X, plate.j_node.Y, plate.j_node.Z, self.origin, self.plane
            )
            xm, ym, zm = _global2local(
                plate.m_node.X, plate.m_node.Y, plate.m_node.Z, self.origin, self.plane
            )
            ...
            for pier in self.piers.values():
                if (
                    round(x_avg, 10) >= round(pier.x, 10)
    ...
    def _identify_coupling_beams(self) -> None:
        ...
        for i in range(len(y_vals) - 1):
            ...
            y = y_vals[i]
            x = 0
            self.coupling_beams["B" + str(i + 1)] = CouplingBeam(
                "B" + str(i + 1), x, y, length, height, self
            )
        ...
        for beam in self.coupling_beams.values():
            for i in range(len(x_vals) - 1):
                ...
                y = beam.y
                x = x_vals[i]
                new_beams["B" + str(beam_count)] = CouplingBeam(
                    "B" + str(beam_count), x, y, length, height, self
                )
                beam_count += 1
        ...
        for beam in self.coupling_beams.values():
            ...
            for opng in self._openings:
                if (
                    round(beam.x, 10) >= round(opng[1], 10)
        ...
        for plate in self.model.quads.values():
            ...
            for beam in self.coupling_beams.values():
                if (
                    round(x_avg, 10) >= round(beam.x, 10)
    ...
    def draw_coupling_beams(
        ...
        for beam in self.coupling_beams.values():
            self._add_rectangle(
                ax, beam.x, beam.y, beam.length, beam.height, beam.name, "white"
            )
        ...
        for i in range(n):
            for j in range(0, n - i - 1):
                if self._openings[j][2] > self._openings[j + 1][2]:
                    self._openings[j], self._openings[j + 1] = (
                        self._openings[j + 1],
                        self._openings[j],
                    )
    ...
    def stiffness(self, story_name: str) -> float:
        ...
        if not any(story[0] == story_name for story in self._stories):
            raise Exception(
                f"Unable to obtain stiffness. Story {story_name} does not exist."
            )
        ...
        # Step through each node in the model
        for node in self.model.nodes.values():
            local_coords = _global2local(
                node.X, node.Y, node.Z, self.origin, self.plane
            )
            x, y, z = local_coords[0], local_coords[1], local_coords[2]

            # Determine if this node is in this story
            if (
                round(x, 10) >= round(story[2], 10)
    ...
    def screenshots(self, combo_name: str = "Combo 1", dir_path: str = "./") -> None:
        ...
        renderer.screenshot(
            dir_path + "/shear_wall_screenshot2.png", interact=False, reset_camera=False
        )
    ...
    def _local2global(
        self, x: float, y: float, z: float, plane: Literal["XY", "XZ", "YZ"] = "XY"
    ) -> list[float]:
...
class CouplingBeam:
    ...
    def sum_forces(
        ...
        for plate in self.plates:
            ...
            xn, yn, zn = _global2local(
                plate.n_node.X, plate.n_node.Y, plate.n_node.Z, self.origin, self.plane
            )
