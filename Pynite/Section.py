...
class Section:
    ...
    def __init__(
        self, model: FEModel3D, name: str, A: float, Iy: float, Iz: float, J: float
    ) -> None:
        """
    ...
    def G(self, fx: float, my: float, mz: float) -> NDArray:
        ...
        dPhi_dfx = (self.Phi(fx + epsilon, my, mz) - self.Phi(fx - epsilon, my, mz)) / (
            2 * epsilon
        )
        dPhi_dmy = (self.Phi(fx, my + epsilon, mz) - self.Phi(fx, my - epsilon, mz)) / (
            2 * epsilon
        )
        dPhi_dmz = (self.Phi(fx, my, mz + epsilon) - self.Phi(fx, my, mz - epsilon)) / (
            2 * epsilon
        )

        # Return the gradient
        return np.array([[dPhi_dfx], [dPhi_dmy], [dPhi_dmz]])
...
class SteelSection(Section):
    ...
    def G(self, fx: float, my: float, mz: float) -> NDArray[float64]:
        ...
        # Plastic action is occuring
            ...
            dPhi_dfx = (
                18 * fx**5 * my**2 / (Mpy**2 * Py**6)
                + 2 * fx / Py**2
                + 7.0 * fx * mz**2 / (Mpz**2 * Py**2)
            )
            dPhi_dmy = (
                6 * fx**6 * my / (Mpy**2 * Py**6)
                + 2 * my / Mpy**2
                + 9.0 * my * mz**4 / (Mpy**2 * Mpz**4)
            )
            dPhi_dmz = (
                7.0 * fx**2 * mz / (Mpz**2 * Py**2)
                + 2 * mz / Mpz**2
                + 18.0 * my**2 * mz**3 / (Mpy**2 * Mpz**4)
            )
