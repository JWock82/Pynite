...
class Renderer:
    ...
    def update(self, reset_camera=True):
        ...
        if self.deformed_shape and self.case != None:
            raise Exception(
                "Deformed shape is only available for load combinations. Deformed shape is not available for load cases."
            )
        if (
        ...
        for spring in self.model.springs.values():
            vis_springs.append(
                VisSpring(spring, self.model.nodes, self.annotation_size)
            )
        ...
        for member in self.model.members.values():
            vis_members.append(
                VisMember(member, self.model.nodes, self.annotation_size, self.theme)
            )
...
class VisNode:
    ...
    def __init__(self, node, annotation_size=5):
        ...
        self.lblActor.SetPosition(
            X + 0.6 * annotation_size, Y + 0.6 * annotation_size, Z
        )
        ...
        if (
            ...
            and node.support_DY == True
            and node.support_DZ == True
            and node.support_RX == True
            and node.support_RY == True
            and node.support_RZ == True
        ):
        ...
        # Check for a pinned support
            ...
            and node.support_DY == True
            and node.support_DZ == True
            and node.support_RX == False
            and node.support_RY == False
            and node.support_RZ == False
        ):
...
class VisDeformedMember:
    def __init__(self, member, nodes, scale_factor, combo_name="Combo 1"):
        ...
        for i in range(20):
            ...
            dx_tot = [[Xi, Yi, Zi]] + (
                L / 19 * i
                + member.deflection("dx", L / 19 * i, combo_name) * scale_factor
            ) * cos_x
...
class VisPtLoad:
    ...
    def __init__(
        ...
        tip.SetCenter(
            position[0] - tip_length * sign * 0.5 * unitVector[0],
        ...
        if label_text != None:
            ...
            self.lblActor.SetScale(annotation_size, annotation_size, annotation_size)
            self.lblActor.SetPosition(
                position[0] - (length - 0.6 * annotation_size) * unitVector[0],
...
class VisDistLoad:
    ...
    def __init__(
        ...
        lineDirCos = [
            (position2[0] - position1[0]) / loadLength,
            (position2[1] - position1[1]) / loadLength,
        ...
        if loadLength > 0:
            num_steps = int(
                round(0.75 * loadLength / max(abs(length1), abs(length2)), 0)
            )
        else:
            num_steps = 0
        ...
        for i in range(num_steps + 1):
            ...
            position = (
                position1[0] + i * step * lineDirCos[0],
                position1[1] + i * step * lineDirCos[1],
            ...
            ptLoads.append(
                VisPtLoad(
                    position, direction, length, label_text, annotation_size, theme
                )
            )
        ...
        tail_line.SetPoint1(
            (
                position1[0] - length1 * dirDirCos[0],
                position1[1] - length1 * dirDirCos[1],
                position1[2] - length1 * dirDirCos[2],
            )
        )
        tail_line.SetPoint2(
            (
                position2[0] - length2 * dirDirCos[0],
                position2[1] - length2 * dirDirCos[1],
...
class VisMoment:
    ...
    def __init__(
        ...
        v1 = direction / norm(
            direction
        )  # v1 = The directional unit vector for the moment
        ...
        self.lblActor.SetScale(annotation_size, annotation_size, annotation_size)
        self.lblActor.SetPosition(
            Xc + v3[0] * (radius + 0.25 * annotation_size),
...
class VisAreaLoad:
    ...
    def __init__(
        ...
        ptLoads = []
        ptLoads.append(
            VisPtLoad(position0, direction, length, label_text, annotation_size, theme)
        )
        ptLoads.append(
            VisPtLoad(position1, direction, length, label_text, annotation_size, theme)
        )
        ptLoads.append(
            VisPtLoad(position2, direction, length, label_text, annotation_size, theme)
        )
        ptLoads.append(
            VisPtLoad(position3, direction, length, label_text, annotation_size, theme)
        )
        ...
        for element in list(model.quads.values()) + list(model.plates.values()):
            ...
            if stress_type == "dz":
                ...
                element.i_node.contour.append(
                    element.moment(r_left, s_bot, True, combo_name)[0]
                )
                element.j_node.contour.append(
                    element.moment(r_right, s_bot, True, combo_name)[0]
                )
                element.m_node.contour.append(
                    element.moment(r_right, s_top, True, combo_name)[0]
                )
                element.n_node.contour.append(
                    element.moment(r_left, s_top, True, combo_name)[0]
                )
            elif stress_type == "My":
                element.i_node.contour.append(
                    element.moment(r_left, s_bot, True, combo_name)[1]
                )
                element.j_node.contour.append(
                    element.moment(r_right, s_bot, True, combo_name)[1]
                )
                element.m_node.contour.append(
                    element.moment(r_right, s_top, True, combo_name)[1]
                )
                element.n_node.contour.append(
                    element.moment(r_left, s_top, True, combo_name)[1]
                )
            elif stress_type == "Mxy":
                element.i_node.contour.append(
                    element.moment(r_left, s_bot, True, combo_name)[2]
                )
                element.j_node.contour.append(
                    element.moment(r_right, s_bot, True, combo_name)[2]
                )
                element.m_node.contour.append(
                    element.moment(r_right, s_top, True, combo_name)[2]
                )
                element.n_node.contour.append(
                    element.moment(r_left, s_top, True, combo_name)[2]
                )
            elif stress_type == "Qx":
                element.i_node.contour.append(
                    element.shear(r_left, s_bot, True, combo_name)[0]
                )
                element.j_node.contour.append(
                    element.shear(r_right, s_bot, True, combo_name)[0]
                )
                element.m_node.contour.append(
                    element.shear(r_right, s_top, True, combo_name)[0]
                )
                element.n_node.contour.append(
                    element.shear(r_left, s_top, True, combo_name)[0]
                )
            elif stress_type == "Qy":
                element.i_node.contour.append(
                    element.shear(r_left, s_bot, True, combo_name)[1]
                )
                element.j_node.contour.append(
                    element.shear(r_right, s_bot, True, combo_name)[1]
                )
                element.m_node.contour.append(
                    element.shear(r_right, s_top, True, combo_name)[1]
                )
                element.n_node.contour.append(
                    element.shear(r_left, s_top, True, combo_name)[1]
                )
            elif stress_type == "Sx":
                element.i_node.contour.append(
                    element.membrane(r_left, s_bot, True, combo_name)[0]
                )
                element.j_node.contour.append(
                    element.membrane(r_right, s_bot, True, combo_name)[0]
                )
                element.m_node.contour.append(
                    element.membrane(r_right, s_top, True, combo_name)[0]
                )
                element.n_node.contour.append(
                    element.membrane(r_left, s_top, True, combo_name)[0]
                )
            elif stress_type == "Sy":
                element.i_node.contour.append(
                    element.membrane(r_left, s_bot, True, combo_name)[1]
                )
                element.j_node.contour.append(
                    element.membrane(r_right, s_bot, True, combo_name)[1]
                )
                element.m_node.contour.append(
                    element.membrane(r_right, s_top, True, combo_name)[1]
                )
                element.n_node.contour.append(
                    element.membrane(r_left, s_top, True, combo_name)[1]
                )
            elif stress_type == "Txy":
                element.i_node.contour.append(
                    element.membrane(r_left, s_bot, True, combo_name)[2]
                )
                element.j_node.contour.append(
                    element.membrane(r_right, s_bot, True, combo_name)[2]
                )
                element.m_node.contour.append(
                    element.membrane(r_right, s_top, True, combo_name)[2]
                )
                element.n_node.contour.append(
                    element.membrane(r_left, s_top, True, combo_name)[2]
                )
...
def _DeformedShape(
    ...
    for spring in model.springs.values():
        ...
        if spring.active[combo_name] == True:
            vis_spring = VisDeformedSpring(
                spring, model.nodes, scale_factor, combo_name
            )
    ...
    for member in model.members.values():
        ...
        if member.active[combo_name] == True:
            vis_member = VisDeformedMember(
                member, model.nodes, scale_factor, combo_name
            )
...
def _RenderLoads(model, renderer, annotation_size, combo_name, case, theme="default"):
    ...
    max_pt_load, max_moment, max_dist_load, max_area_load = _MaxLoads(
        model, combo_name, case
    )
    ...
    for node in model.nodes.values():
        ...
        for load in node.NodeLoads:
            ...
            if load[2] in load_factors:
                ...
                if load[0] == "FX":
                    ptLoad = VisPtLoad(
                        (node.X - 0.6 * annotation_size * sign, node.Y, node.Z),
                        [1, 0, 0],
                        load_value / max_pt_load * 5 * annotation_size,
                        f"{load_value:.3g}",
                        annotation_size,
                        theme,
                    )
                    ...
                    ptLoad = VisPtLoad(
                        (node.X, node.Y - 0.6 * annotation_size * sign, node.Z),
                        [0, 1, 0],
                        load_value / max_pt_load * 5 * annotation_size,
                        f"{load_value:.3g}",
                        annotation_size,
                    ...
                    ptLoad = VisPtLoad(
                        (node.X, node.Y, node.Z - 0.6 * annotation_size * sign),
                        [0, 0, 1],
                        load_value / max_pt_load * 5 * annotation_size,
                        f"{load_value:.3g}",
                        annotation_size,
                    ...
                    ptLoad = VisMoment(
                        ...
                        (1 * sign, 0, 0),
                        abs(load_value) / max_moment * 2.5 * annotation_size,
                        f"{load_value:.3g}",
                        annotation_size,
                        theme,
                    )
    ...
    for member in model.members.values():
        ...
        for load in member.DistLoads:
            ...
            if load[5] in load_factors:
                ...
                # Display the load
                if load[0] == "Fx":
                    distLoad = VisDistLoad(
                        position1,
                        position2,
                        dir_cos[0, :],
                        w1 / max_dist_load * 5 * annotation_size,
                        w2 / max_dist_load * 5 * annotation_size,
                        f"{w1:.3g}",
                        f"{w2:.3g}",
                        annotation_size,
                    )
    ...
    for plate in list(model.plates.values()) + list(model.quads.values()):
        ...
        for load in plate.pressures:
            ...
            if load[1] in load_factors:
                ...
                area_load = VisAreaLoad(
                    position0,
                    position1,
                    position2,
                    position3,
                    dir_cos * sign,
                    abs(load_value) / max_area_load * 5 * annotation_size,
                    f"{load_value:.3g}",
                    annotation_size,
                    theme,
                )
                ...
                polygon_points.InsertNextPoint(
                    area_load.p0[0], area_load.p0[1], area_load.p0[2]
                )
                polygon_points.InsertNextPoint(
                    area_load.p1[0], area_load.p1[1], area_load.p1[2]
                )
                polygon_points.InsertNextPoint(
                    area_load.p2[0], area_load.p2[1], area_load.p2[2]
                )
                polygon_points.InsertNextPoint(
                    area_load.p3[0], area_load.p3[1], area_load.p3[2]
                )
            ...
            for load in node.NodeLoads:
                ...
                if load[2] in model.load_combos[combo_name].factors:
                    if load[0] == "FX" or load[0] == "FY" or load[0] == "FZ":
                        if (
                            abs(
                                load[1] * model.load_combos[combo_name].factors[load[2]]
                            )
                            > max_pt_load
                        ):
                            max_pt_load = abs(
        ...
        for member in model.members.values():
            ...
            for load in member.PtLoads:
                ...
                if load[3] in model.load_combos[combo_name].factors:
                    if (
                        ...
                        if (
                            abs(
                                load[1] * model.load_combos[combo_name].factors[load[3]]
                            )
                            > max_pt_load
                        ):
                            max_pt_load = abs(
            ...
            for load in member.DistLoads:
                ...
                if load[5] in model.load_combos[combo_name].factors:
                    if (
                        abs(load[1] * model.load_combos[combo_name].factors[load[5]])
                        > max_dist_load
                    ):
                        max_dist_load = abs(
                            load[1] * model.load_combos[combo_name].factors[load[5]]
                        )
                    if (
                        abs(load[2] * model.load_combos[combo_name].factors[load[5]])
                        > max_dist_load
                    ):
                        max_dist_load = abs(
                            load[2] * model.load_combos[combo_name].factors[load[5]]
                        )
        ...
        for plate in model.plates.values():
            ...
            for load in plate.pressures:
                if load[1] in model.load_combos[combo_name].factors:
                    if (
                        abs(load[0] * model.load_combos[combo_name].factors[load[1]])
                        > max_area_load
                    ):
                        max_area_load = abs(
                            load[0] * model.load_combos[combo_name].factors[load[1]]
                        )
