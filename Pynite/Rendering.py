...
class Renderer:
    ...
    def render_model(self, reset_camera: bool = True) -> None:
        ...
        self.plotter.show(
            title="Pynite - Simple Finite Element Analysis for Python", auto_close=False
        )

    def screenshot(
        self,
        filepath: str = "./Pynite_Image.png",
        interact: bool = True,
            ...
            for func in self.post_update_callbacks:
                if callable(func):
                    try:
                        ...
                        warnings.warn(
                            f"Error executing post-update callback {func.__name__}: {e}"
                        )
                else:
                    warnings.warn(
                        f"Item in post_update_callbacks is not callable: {func}"
                    )
    ...
    def plot_spring(
        self, spring: Spring3D, color: str = "grey", deformed: bool = False
    ) -> None:
        """
        ...
        if np.allclose(direction, arbitrary_vector) or np.allclose(
            direction, -arbitrary_vector
        ):
        ...
        # Combine the points
        points = np.vstack(
            [start_point, start_segment, zigzag_points, end_segment, end_point]
        )
    ...
    def plot_plates(
        ...
        if color_map:
            ...
            if self._scalar_bar == True:
                self.plotter.add_mesh(
                    plate_polydata, scalars="Contours", show_edges=True
                )
            else:
    ...
    def plot_deformed_node(
        self, node: Node3D, scale_factor: float, color: str = "grey"
    ) -> None:
    ...
    def plot_pt_load(
        ...
        length: float,
        label_text: str | float | int | None = None,
        color: str = "green",
    ) -> None:
    ...
    def plot_dist_load(
        ...
        length1: float,
        length2: float,
        label_text1: str | float | int | None,
        label_text2: str | float | int | None,
        color: str = "green",
    ) -> None:
        ...
        tail_line = pv.Line(
            position1 - dir_dir_cos * length1, position2 - dir_dir_cos * length2
        )
    ...
    def plot_moment(
        ...
        arc = pv.CircularArcFromNormal(
            center, resolution=20, normal=v1, angle=215, polar=v2 * radius
        )
...
def sig_fig_round(number, sig_figs):
    ...
    if not isinstance(number, (float, int)):
        try:
            ...
            raise ValueError(
                f"{number} is not a number. Ensure that `number` is numeric."
            )
