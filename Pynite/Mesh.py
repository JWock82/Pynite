...
class Mesh:
    ...
    def __init__(
        ...
        self.kx_mod = (
            kx_mod  # Local x stiffness modification factor for elements in the mesh
        )
        self.ky_mod = (
            ky_mod  # Local y stiffness modification factor for elements in the mesh
        )
        ...
        self.nodes: dict[
            str, Node3D
        ] = {}  # A dictionary containing the nodes in the mesh
        self.elements: dict[
            str, Quad3D | Plate3D
        ] = {}  # A dictionary containing the elements in the mesh
        self.element_type = "Quad"  # The type of element used in the mesh
        self.is_generated = (
            False  # A flag indicating whether the mesh has been generated
        )
    ...
    def max_shear(
        self, direction: str = "Qx", combo_tags: str | list[str] = "Combo 1"
    ) -> float:
    ...
    def max_moment(
        self, direction: str = "Mx", combo_tags: str | list[str] = "Combo 1"
    ) -> float:
    ...
    def max_membrane(
        self, direction: str = "Sx", combo_tags: str | list[str] = "Combo 1"
    ) -> float:
...
class RectangleMesh(Mesh):
    def __init__(
        ...
        plane: str = "XY",
        x_control: list[float] | None = None,
        y_control: list[float] | None = None,
        start_node: str = "N1",
        start_element: str = "Q1",
        ...
        super().__init__(
            thickness, material_name, model, kx_mod, ky_mod, start_node, start_element
        )
    ...
    def generate(self) -> None:
        ...
        if element_type == "Quad":
            ...
            raise Exception(
                "Invalid element type specified for RectangleMesh. Select 'Quad' or 'Rect'."
            )
        ...
        for i in range(1, num_cols * num_rows + 1, 1):
            ...
            if element_type == "Quad":
                self.elements[element_name] = Quad3D(
                    element_name,
                    self.nodes["N" + str(i_node + node_offset)],
                ...
                self.elements[element_name] = Plate3D(
                    element_name,
                    self.nodes["N" + str(i_node + node_offset)],
                    self.nodes["N" + str(j_node + node_offset)],
                    self.nodes["N" + str(m_node + node_offset)],
        ...
        for node in self.nodes.values():
            ...
            for opng in self.openings.values():
                ...
                if (
                    round(x, 10) > round(opng.x_left, 10)
        ...
        for element in self.elements.values():
            ...
            for opng in self.openings.values():
                ...
                if (
                    (round(opng.y_bott + opng.height, 10) >= round(top, 10))
        ...
        for node in self.nodes.values():
            if (
                node not in [element.i_node for element in self.elements.values()]
    ...
    def add_rect_opening(
        self, name: str, x_left: float, y_bott: float, width: float, height: float
    ) -> None:
...
class RectOpening:
    ...
    def __init__(
        self, x_left: float, y_bott: float, width: float, height: float
    ) -> None:
...
class AnnulusMesh(Mesh):
    ...
    def generate(self) -> None:
        ...
        n_circ = int(
            circumf / mesh_size
        )  # Number of times `mesh_size` fits in the circumference
        ...
        while round(r_inner, 10) < round(r_outer, 10):
            radial = (
                r_outer - r_inner
            )  # Remaining length in the radial direction to be meshed
            circumf = 2 * pi * r_inner  # Circumference of the ring at the inner radius
            b_circ = circumf / n_circ  # Element width in the circumferential direction
            n_rad = int(
                radial / min(mesh_size, 3 * b_circ)
            )  # Number of times the plate width fits in the remaining unmeshed radial direction
...
class AnnulusTransRingMesh(Mesh):
    ...
    def generate(self) -> None:
        ...
        theta3 = (
            2 * pi / (self.n * 3)
        )  # Angle between nodes at the outer radius of the ring
...
class CylinderMesh(Mesh):
    ...
    def generate(self) -> None:
        ...
        num_elements = (
            self.num_elements
        )  # Number of quadrilaterals in each course of the ring
...
class CylinderRingMesh(Mesh):
    ...
    def generate(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                "Invalid node name. Enter a letter followed by a number (e.g. 'N25')"
            )
        ...
        try:
            ...
            raise ValueError(
                "Invalid element ame. Enter a letter followed by a number (e.g. 'Q83')"
            )
        ...
        for i in range(1, n + 1, 1):
            ...
            if self.element_type == "Quad":
                ...
                raise Exception(
                    "Invalid element type specified for cylinder ring mesh."
                )
...
def check_mesh_integrity(
    mesh: Mesh, console_log: bool = True
) -> str | list[str] | None:
    ...
    for element in mesh.elements.values():
        if (
            element.i_node not in mesh.nodes.values()
            or element.j_node not in mesh.nodes.values()
            or element.m_node not in mesh.nodes.values()
            or element.n_node not in mesh.nodes.values()
        ):
            count += 1
    ...
    if count != 0:
        errors.append(
            str(count) + " elements are attached to nodes that are not in the mesh."
        )
    ...
    for element in mesh.elements.values():
        if (
            element.i_node not in mesh.model.nodes.values()
            or element.j_node not in mesh.model.nodes.values()
            or element.m_node not in mesh.model.nodes.values()
            or element.n_node not in mesh.model.nodes.values()
        ):
            count += 1
    ...
    if count != 0:
        errors.append(
            str(count) + " elements are attached to nodes that are not in the model."
        )
    ...
    for element in mesh.elements.values():
        if (
            element.i_node is element.j_node
            or element.i_node is element.m_node
            or element.i_node is element.n_node
            or element.j_node is element.m_node
            or element.j_node is element.n_node
            or element.m_node is element.n_node
        ):
            count += 1
    ...
    for element in mesh.elements.values():
        if (
            element.name not in mesh.model.plates.keys()
            and element.name not in mesh.model.quads.keys()
        ):
            count += 1

    if count != 0:
        errors.append(
            str(count) + " element names in the mesh were not found in the model."
        )
    ...
    for element in mesh.elements.values():
        if (
            element not in mesh.model.plates.values()
            and element.name not in mesh.model.quads.values()
        ):
            count += 1
    ...
    for element in mesh.elements.values():
        if mesh.element_type == "Rect":
            if (
                element.name not in mesh.model.plates.keys()
                or mesh.elements[element.name] is not mesh.model.plates[element.name]
            ):
                count += 1
        elif mesh.element_type == "Quad":
            if (
                element.name not in mesh.model.quads.keys()
                or mesh.elements[element.name] is not mesh.model.quads[element.name]
            ):
                count += 1
    ...
    if count != 0:
        errors.append(
            str(count)
            + " elements in the mesh do not match their respective elements in the model."
        )
    ...
    if count != 0:
        errors.append(
            str(count)
            + " node names in the mesh do not match the mesh's `nodes` dictionary's keys."
        )
