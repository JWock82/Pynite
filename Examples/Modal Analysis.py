...
from Pynite.Visualization import (
    Renderer,
)  # Using the VTK renderer for its speed, but we could also use the PyVista renderer the same way

rndr = Renderer(model)  # Create a new renderer based on the model
rndr.annotation_size = (
    1  # Text will display 1' high, and objects (e.g. supports) will scale accordingly
)
